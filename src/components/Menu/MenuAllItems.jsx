...
const MenuAllItems = ({element}) => {
  ...
  return (
<>
{element.title && <div className="items d-flex justify-content-between my-3 p-2" style={{"boxShadow": "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px"}}>
    <h5>
      {element?.title}({element?.itemCards?.length})
    </h5>
   {!showMenu ? <IoIosArrowDropdown size={30} onClick={() => setShowMenu(true)}/> :
